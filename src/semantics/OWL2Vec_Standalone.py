...
CONFIG_FILE = os.path.join(os.path.dirname(os.path.realpath(__file__)), "../../owl2vec/default.cfg")
    ...
    if 'Lit_Doc' in config['DOCUMENT'] and config['DOCUMENT']['Lit_Doc'] == 'yes':
        ...
        for annotation in annotations:
            ...
            if len(processed_words) > 0:
                Lit_Doc.append(label_item(uri_label, item=annotation[0]) + processed_words)
        print('Extracted %d annotation sentences' % len(Lit_Doc))

        for sentence in walk_sentences:
            lit_sentence = list()
            for item in sentence:
                lit_sentence += label_item(uri_label, item=item)
            ...
            if config['DOCUMENT']['Mix_Type'] == 'all':
                for index in range(len(sentence)):
                    ...
                    for i, item in enumerate(sentence):
                        mix_sentence += [item] if i == index else label_item(uri_label, item=item)
                    Mix_Doc.append(mix_sentence)
            elif config['DOCUMENT']['Mix_Type'] == 'random':
                random_index = random.randint(0, len(sentence) - 1)
                mix_sentence = list()
                for i, item in enumerate(sentence):
                    mix_sentence += [item] if i == random_index else label_item(uri_label, item=item)
                Mix_Doc.append(mix_sentence)
